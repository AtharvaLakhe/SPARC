...
|---|---|---:|---:|---|---|---|
| R-DATA-01 | Earth Engine access, quota or catalogue calls fail during offline discovery/processing | Medium | High | Codex | Day 0 metadata inventory; bounded worker requests; cache stable image IDs; direct-CDSE fallback | Use direct CDSE discovery where needed, then retained local inputs; judged path uses precomputed pack |
...
| R-DATA-05 | Boundary geometry, names or hierarchy fail QA | Medium | High | Codex | geoBoundaries ADM2 feature ID/name, WGS84 CRS, polygon validity, point-in-ADM1 state location, archive/output hashes and provenance gate | Keep district only; Hingna remains merely a provisional candidate until it passes; select another validated Nagpur child only with documented QA |
| R-LIC-01 | Boundary redistribution terms are restrictive or misdescribed | Medium | Critical | Shared | Use the selected India ADM2 release’s ODbL 1.0 record, attribution, and applicable share-alike obligations; do not treat collection-level CC BY wording as a CC-BY-only grant | Do not redistribute a boundary unless the bundle and deployment meet ODbL conditions; use another verified source or omit geometry; no Survey of India ABDB geometry is used |
...
| R-METH-01 | MNDWI threshold is unstable across sensitivity runs | Medium | High | Codex | Fixed rule, pooled calibration, sensitivity and confusion-risk review. Bengaluru's fixed-zero and pooled-Otsu rules have the same positive direction (+9.22/+96.17 km²) but materially different magnitude | Show low/medium quality and sensitivity; use simpler frozen rule; withhold policy-facing water claim if conclusion changes or independent validation is absent |
...
| R-METH-03 | Built-up spectral proxy confuses bare soil, construction or bright roofs | High | High | Codex | Exclusions, fixed calibration, risk strata, sensitivity, visual review, and separate blinded frames for the constrained-NDBI and IBI v2 rules | **Observed 2026-08-03:** constrained-NDBI and IBI v2 sensitivity reverse Nagpur's net direction (+158.47 versus −361.52 km²). Two 100-point exploratory frames now exist, but have no independent labels or probability design. Bengaluru's two rules are both negative (−79.18/−5.08 km²), which does not remove the accuracy gate. Withhold any built-change finding; retain only an explicitly unstable pre-publication diagnostic pending independent validation and method review |
...
|---|---|---|---|---|
| Bhuvan unavailable or permission unclear | Do not depend on Bhuvan; use the approved GEE Sentinel and boundary sources | Use a specifically identified Bhuvan dataset only after its own permission is verified | Retain already approved, attributable outputs only | Omit Bhuvan context; core dashboard remains complete |
| Google Earth Engine unavailable or an interactive reduction times out | Stop the offline worker within its retry budget; move expensive 10 m reductions to a controlled batch/export rather than relaxing scale or QA | Direct CDSE metadata/discovery fallback preserving Sentinel identity | Retained local inputs and independently verified immutable district results | Local demo pack with no provider call |
| Copernicus/CDSE unavailable | Continue GEE processing when catalogue access is healthy | Retained selected GEE image IDs/results | Independently verified immutable district results | Local demo pack with no provider call |
...
- [x] Provider/product metadata and district boundary terms are known enough to begin controlled P0 processing; source-specific ODbL obligations remain a release check.
