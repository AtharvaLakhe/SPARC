...
| Google Earth Engine unavailable | Stop the offline worker within its retry budget | Direct CDSE metadata/discovery fallback preserving Sentinel identity | Retained local inputs and independently verified immutable district results | Local demo pack with no provider call |
