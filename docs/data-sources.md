...
- **DECISION:** Bengaluru Urban backup periods are 2019-01-15 through 2019-03-15 and the same dates in 2024. Its district boundary, catalog, and common-valid-area gates passed on 2026-08-03; do not change these dates silently.
...
- **FACT:** the 2026-08-02 metadata-only discovery found candidate Sentinel-2 L2A products with all required assets in every fixed pilot window. The geoBoundaries ADM2 district gate has passed for Nagpur and the Bengaluru Urban backup. Nagpur's downloaded 2026-08-03 water pooled-Otsu and built IBI v2 sensitivity CSVs passed local boundary-checksum, CRS, method-setting, coverage, and area-arithmetic validation. The fixed-zero and pooled-Otsu water results have the same net-loss direction (−8.50 and −10.16 km²) but still require independent validation. The constrained-NDBI and IBI built-proxy results reverse direction (+158.47 and −361.52 km²), so no Nagpur built-change finding may be used beyond an explicitly unstable, pre-publication diagnostic. Bengaluru Urban's water pooled-Otsu, vegetation 0.20/0.30/0.40, and built IBI sensitivity CSVs have now passed the same local checks. Its fixed-zero/pooled-Otsu water outputs are both positive (+9.22/+96.17 km²); its constrained-NDBI/IBI diagnostics are both negative (−79.18/−5.08 km²); and its vegetation sensitivities are all positive (+30.51/+58.03/+51.76 km²). Directional agreement is sensitivity evidence only, not validation or an accuracy finding. The 10 m vegetation reductions completed as full-resolution Google Earth Engine CSV batch tasks in the approved Drive folder. All P0 outputs remain `quality: unknown` pending independent validation. See the [pilot source gate](research/pilot-source-gate.md).
