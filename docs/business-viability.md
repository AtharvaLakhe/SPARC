...
| Earth Engine/custom notebook workflow | Broad catalog and scalable exploratory analysis for skilled users | SPARC uses Earth Engine only in its offline worker; the judged path uses frozen local outputs and a public contract | Do not duplicate mature processing/catalog functionality or expose provider credentials to users |
