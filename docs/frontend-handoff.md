...
**Status:** React analytical panel integrated into the globe-led `orbital-website/` runtime. See the [delivery status](./project-status.md) for current readiness and remaining scope.
...
At this checkpoint, the repository contains contracts, synthetic fixtures, a mock-only API, processing evidence, and a combined Three.js targeting experience with the React analytical panel mounted over it. Paths below marked “planned” must not be described as current code.
...
| Browser/client, existing | [`orbital-website/`](../orbital-website/README.md) | Browser over local HTTP | Globe-led SPARC entry, offline place/coordinate targeting, and the panel handoff |
| Browser/client, existing | `apps/web/**` | Browser after Vite build | React analytical panel, browser-side input checks, data gateway, view-model mapping and accessibility |
| Build/configuration, existing | `apps/web/vite.config.*`, TypeScript configuration | Developer machine/CI | Bundles the panel and injects public `VITE_*` values |
